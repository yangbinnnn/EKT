...
func (api *Api) Block_new(body *[]byte) ([]byte, *tp.Rerror) {
	...
	// sess init
	cs := blockchain_manager.GetMainChainConsensus()
	for _, peer := range block.GetRound().Peers {
		sessPeer := cs.GetPeer(peer.PeerId)
		if sessPeer == nil && !peer.Equal(conf.EKTConfig.Node) || !sessPeer.Online() {
			// unexcpeted to here
			log.Debug("new block, peer dial sess")
			go peer.Dial()
		} else {
			log.Debug("new block, peer copy sess")
			peer.CopySess(sessPeer)
		}
	}
